...
namespace AlephNote.Plugins.StandardNote
{
	...
	public class StandardNoteConnection : BasicRemoteConnection
	{
		...
		public override void OnAfterSyncError(INoteRepository repo, Exception e)
		{
			if (e is RestStatuscodeException rse && rse.StatusCode == 401)
			{
				var data = (StandardNoteData)repo.GetSyncData();

				_logger.Warn(StandardNotePlugin.Name,
					"Restart Authentication flow (401 statuscode)",
					$"Exception              := {rse.GetType()}\n" +
					$"Message                := {rse.Message}\n" +
					$"Source                 := {rse.Source}\n" +
					$"StatusCode             := {rse.StatusCode}\n" +
					$"StatusPhrase           := {rse.StatusPhrase}\n" +
					$"\n" +
					$"StackTrace             := \n{rse.StackTrace}\n" +
					$"\n" +
					$"HTTPContent            := \n{rse.HTTPContent}\n" +
					$"\n" +
					$"Old[RefreshToken]      := {data.SessionData.RefreshToken}\n" +
					$"Old[RefreshExpiration] := {data.SessionData.RefreshExpiration?.ToString("yyyy-MM-dd HH:mm:ss") ?? "NULL"}\n" +
					$"Old[AccessExpiration]  := {data.SessionData.AccessExpiration?.ToString("yyyy-MM-dd HH:mm:ss") ?? "NULL"}\n" +
					$"Old[Token]             := {data.SessionData.Token}\n");
